  ...
  <div class="container" style="margin-left:90px;">
    ...
    <div class="navbar-collapse collapse justify-content-end" id="navbarDefault">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link js-scroll photodas" href="/wedding/dasboard.php">Dashboard</a>
        </li>
      </ul>
    </div>
  </div>
...
<!--/ Nav End /-->
                ...
                <div class="signup-content">
                    <form method="POST" id="signup-form" class="signup-form">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-input" name="vname" id="name" placeholder="PhotoShop Name" required/>
                        </div>
                        <div class="form-group">
    ...
    <h2 class="h2">Photoshop Details</h2>
    ...
    if (isset($_POST['delete_pid'])) {
        $id_to_delete = $_POST['delete_pid'];

        // Prepare the delete query
        $delete_sql = "DELETE FROM photoshop WHERE pid=?";
...
if (isset($_POST['update_pid'])) {
    ...
    $update_sql = "UPDATE photoshop SET name=?, price=?, descr=? WHERE pid=?";
    
    // Execute the update query using prepared statements
    if ($stmt = $con->prepare($update_sql)) {
        $stmt->bind_param("sdsi", $updated_name, $updated_price, $updated_descr, $pid);
        $stmt->execute();
        $stmt->close();
...
// SQL query to fetch data from the photoshop table
$search = isset($_GET['search']) ? $_GET['search'] : '';

$sql = "SELECT pid, name, price, descr FROM photoshop";
if (!empty($search)) {
    $sql .= " WHERE name LIKE ?";
}
...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
      ...
      echo "<button type='submit' name='delete_photoshop' class=' btn btn-outline-danger' style='padding: 8px 15px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Remove</button>";
      echo "</form>";

      // Edit Button
      echo "<button class='btn btn-outline-primary editBtn ' data-pid='" . $row['pid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "'
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('modal_edit_pid').value = pid;
...
<div id="editModal" class="modal">
  <div class="modal-content">
    ...
    <form method="POST" class="edit-venue-form">
      <input type="hidden" name="pid" id="modal_edit_pid">
      ...
      <div style="text-align: center;">
        <button type="submit" name="update_pid" class="btn btn-primary" style="padding: 12px 25px; border-radius: 8px; background-color: black; color: #ffffff; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border: none; transition: background-color 0.3s ease;">
          Update
        </button>
      </div>
    </form>
