  ...
  <div class="container" style="margin-left:90px;">
    ...
    <div class="navbar-collapse collapse justify-content-end" id="navbarDefault">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link js-scroll musicdas" href="/wedding/dasboard.php">Dashboard</a>
        </li>
      </ul>
    </div>
  </div>
                ...
                <div class="signup-content">
                    <form method="POST" id="signup-form" class="signup-form">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-input" name="vname" id="name" placeholder="Music Name" required />
                        </div>
                        <div class="form-group"> 
...
if (isset($_POST['delete_mid'])) {
    $id_to_delete = $_POST['delete_mid'];
    
    // Prepare the delete query
    $delete_sql = "DELETE FROM music WHERE mid=?";
...
// update the record form admin
    if (isset($_POST['update_mid'])) {
        $mid = $_POST['mid'];
        ...
        $update_sql = "UPDATE music SET name=?, price=?, descr=? WHERE mid=?";
        
        // Execute the update query using prepared statements
        if ($stmt = $con->prepare($update_sql)) {
            $stmt->bind_param("sdsi", $updated_name, $updated_price, $updated_descr, $mid);
            $stmt->execute();
            $stmt->close();
    ...
    $sql = "SELECT mid, name, price, descr FROM music";
    if (!empty($search)) {
        $sql .= " WHERE name LIKE ?";
    }
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<input type='hidden' name='delete_mid' value='" . $row['mid'] . "'>";
        echo "<button type='submit' name='delete_music' class=' btn btn-outline-danger' style='padding: 8px 15px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Remove</button>";
        echo "</form>";
  
        // Edit Button
        echo "<button class='btn btn-outline-primary editBtn ' data-mid='" . $row['mid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "'
  ...
  @keyframes slideDown {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editButtons.forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                document.getElementById('modal_edit_mid').value = mid;
    ...
    <div id="editModal" class="modal">
      <div class="modal-content">
        ...
        <h3 class="venueh3">Edit Music</h3>
        <form method="POST" class="edit-venue-form">
          <input type="hidden" name="mid" id="modal_edit_mid">
          ...
          <div style="text-align: center;">
            <button type="submit" name="update_mid" class="btn btn-primary" style="padding: 12px 25px; border-radius: 8px; background-color: black; color: #ffffff; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border: none; transition: background-color 0.3s ease;">
              Update
            </button>
          </div>
        </form>
