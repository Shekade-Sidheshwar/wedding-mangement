  ...
  <div class="container" style="margin-left:90px;">
    ...
    <div class="navbar-collapse collapse justify-content-end" id="navbarDefault">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link js-scroll venudas" href="/wedding/dasboard.php">Dashboard</a>
        </li>
      </ul>
    </div>
  </div>
                ...
                <div class="signup-content">
                    <form method="POST" id="signup-form" class="signup-form">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-input" name="vname" id="name" placeholder="Venue Name" required/>
                        </div>
                        <div class="form-group">
...
if (isset($_POST['delete_vid'])) {
    $id_to_delete = $_POST['delete_vid'];
    
    // Prepare the delete query
    $delete_sql = "DELETE FROM venue WHERE vid=?";
...
if (isset($_POST['update_vid'])) {
    ...
    $update_sql = "UPDATE venue SET name=?, price=?, descr=? WHERE vid=?";
    
    // Execute the update query using prepared statements
    if ($stmt = $con->prepare($update_sql)) {
        $stmt->bind_param("sdsi", $updated_name, $updated_price, $updated_descr, $vid);
        $stmt->execute();
        $stmt->close();
...
// $sql = "SELECT vid, name, price, descr FROM venue";
...
//       echo "<input type='hidden' name='delete_vid' value='" . $row['vid'] . "'>";
//       echo "<button type='submit' name='delete_venue' class=' btn btn-outline-danger' style='padding: 8px 15px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Remove</button>";
//       echo "</form>";

//       // Edit Button
//       echo "<button class='btn btn-outline-primary editBtn ' data-vid='" . $row['vid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "'
...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
      ...
      // Edit Button
      echo "<button class='btn btn-outline-primary editBtn ' data-vid='" . $row['vid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "' style='padding: 8px 15px;margin-left:22px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Edit</button>";
  ...
  echo "<div style='text-align:center; font-size: 40px; color: #ffff; padding: 20px; font-style: italic;'>No results found</div>";
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('modal_edit_vid').value = vid;
...
<div id="editModal" class="modal">
  <div class="modal-content">
    ...
    <form method="POST" class="edit-venue-form">
      <input type="hidden" name="vid" id="modal_edit_vid">

      <div style="display: flex; justify-content: space-between; margin-bottom: 25px;">
        <div style="flex: 1; margin-right: 15px;">
          <label for="modal_edit_name" style="color: black; font-size: 14px; font-weight: bold; ">Name:</label>
          <input type="text" name="name" id="modal_edit_name" required placeholder="Enter venue name"  class="venuedit">
        </div>
      ...
      <div style="text-align: center;">
        <button type="submit" name="update_vid" class="btn btn-primary" style="padding: 12px 25px; border-radius: 8px; background-color: black; color: #ffffff; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border: none; transition: background-color 0.3s ease;">
          Update
        </button>
      </div>
    </form>
