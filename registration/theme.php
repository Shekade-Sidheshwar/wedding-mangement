  ...
  <div class="container" style="margin-left:90px;">
    ...
    <div class="navbar-collapse collapse justify-content-end" id="navbarDefault">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link js-scroll themedas" href="/wedding/dasboard.php">Dashboard</a>
        </li>
      </ul>
    </div>
  </div>
...
<div class="container" style="margin-top: 7px; display: flex; justify-content: right;margin-left:64%;margin-bottom:-15px;">
    ...
    // SQL query to fetch data from the theme table
    $sql = "SELECT tid, name, price, descr FROM theme";
    $result = $con->query($sql);

    // Check if there are results
    if (isset($_POST['update_tid'])) {
        $tid = $_POST['tid'];
        ...
        $update_sql = "UPDATE theme SET name=?, price=?, descr=? WHERE tid=?";
        
        // Execute the update query using prepared statements
        if ($stmt = $con->prepare($update_sql)) {
            $stmt->bind_param("sdsi", $updated_name, $updated_price, $updated_descr, $tid);
            $stmt->execute();
            $stmt->close();
...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
      ...
      echo "<input type='hidden' name='delete_tid' value='" . $row['tid'] . "'>";
      echo "<button type='submit' name='delete_theme' class=' btn btn-outline-danger' style='padding: 8px 15px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Remove</button>";
      echo "</form>";

      // Edit Button
      echo "<button class='btn btn-outline-primary editBtn ' data-tid='" . $row['tid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "'
    ...
    <div id="editModal" class="modal">
      <div class="modal-content">
        ...
        <form method="POST" class="edit-venue-form">
          <input type="hidden" name="tid" id="modal_edit_tid">
          ...
          <div style="text-align: center;">
            <button type="submit" name="update_tid" class="btn btn-primary" style="padding: 12px 25px; border-radius: 8px; background-color: black; color: #ffffff; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border: none; transition: background-color 0.3s ease;">
              Update
            </button>
          </div>
        </form>
