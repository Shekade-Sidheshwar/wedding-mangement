  ...
  <div class="container" style="margin-left:90px;">
    ...
    <div class="navbar-collapse collapse justify-content-end" id="navbarDefault">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link js-scroll catdas" href="/wedding/dasboard.php">Dashboard</a>
        </li>
      </ul>
    </div>
  </div>
...
<!--/ Nav End /-->
                ...
                <div class="signup-content">
                    <form method="POST" id="signup-form" class="signup-form">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-input" name="vname" id="name" placeholder="Catering Name" required/>
                        </div>
                        <div class="form-group">
    ...
    <h2 class="h2">Catering Details</h2>
    ...
    if (isset($_POST['delete_cid'])) {
        $id_to_delete = $_POST['delete_cid'];

        // Prepare the delete query
        $delete_sql = "DELETE FROM catering WHERE cid=?";
...
if (isset($_POST['update_cid'])) {
    ...
    $update_sql = "UPDATE catering SET name=?, price=?, descr=? WHERE cid=?";
    
    // Execute the update query using prepared statements
    if ($stmt = $con->prepare($update_sql)) {
        $stmt->bind_param("sdsi", $updated_name, $updated_price, $updated_descr, $cid);
        $stmt->execute();
        $stmt->close();
...
$sql = "SELECT cid, name, price, descr FROM catering";
if (!empty($search)) {
    $sql .= " WHERE name LIKE ?";
}
...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
      ...
      echo "<button type='submit' name='delete_catering' class=' btn btn-outline-danger' style='padding: 8px 15px; border-radius: 5px; transition: background-color 0.3s; font-weight: bold;'>Remove</button>";
      echo "</form>";

      // Edit Button
      echo "<button class='btn btn-outline-primary editBtn ' data-cid='" . $row['cid'] . "' data-name='" .$row['name'] . "' data-price='" . $row['price'] . "' data-descr='" . addslashes($row['descr']) . "'
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('modal_edit_cid').value = cid;
...
<div id="editModal" class="modal">
  <div class="modal-content">
    ...
    <form method="POST" class="edit-venue-form">
      <input type="hidden" name="cid" id="modal_edit_cid">
      ...
      <div style="text-align: center;">
        <button type="submit" name="update_cid" class="btn btn-primary" style="padding: 12px 25px; border-radius: 8px; background-color: black; color: #ffffff; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border: none; transition: background-color 0.3s ease;">
          Update
        </button>
      </div>
    </form>
